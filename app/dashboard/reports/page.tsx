...
import { Download, Filter, Calendar, TrendingUp, DollarSign, Users, Building2, AlertTriangle, BarChart3, PieChart, FileText, Eye } from 'lucide-react';
...
export default function ReportsPage() {
  ...
  const lateTasks = [
    {
      ...
      description: language === 'ar' ? 'عقد أحمد العتيبي - A101' : 'Ahmed Al-Otaibi Contract - A101',
      dueDate: '2024-01-20',
      daysOverdue: 2,
    ...
    },
    {
      ...
      description: language === 'ar' ? 'دفعة خالد المطيري - V15' : 'Khalid Al-Mutairi Payment - V15',
      dueDate: '2024-01-18',
      daysOverdue: 4,
    ...
    },
    {
      ...
      description: language === 'ar' ? 'طلب صيانة نورا السالم - C302' : 'Nora Al-Salem Maintenance - C302',
      dueDate: '2024-01-19',
      daysOverdue: 3,
  ...
  const tabs = [
    { id: 'sales', label: language === 'ar' ? 'تقارير المبيعات' : 'Sales Reports' },
    { id: 'payments', label: language === 'ar' ? 'تقارير المدفوعات' : 'Payment Reports' },
    { id: 'satisfaction', label: language === 'ar' ? 'رضا العملاء' : 'Customer Satisfaction' },
    { id: 'projects', label: language === 'ar' ? 'أداء المشاريع' : 'Project Performance' },
    { id: 'tasks', label: language === 'ar' ? 'المهام المتأخرة' : 'Late Tasks' }
  ];
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-stone-gray mt-1">
              {language === 'ar' ? 'تقارير شاملة وتحليلات الأداء' : 'Comprehensive reports and performance analytics'}
            </p>
          </div>
          <div className="flex space-x-3 rtl:space-x-reverse">
            ...
            >
              ...
              <span>{language === 'ar' ? 'تحميل التقارير' : 'Download Reports'}</span>
            </motion.button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-stone-gray text-sm">{language === 'ar' ? 'إجمالي المبيعات' : 'Total Sales'}</p>
                <p className="text-2xl font-bold text-elegant-white">5.2M</p>
                <p className="text-green-400 text-sm">+12% {language === 'ar' ? 'من الشهر الماضي' : 'from last month'}</p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-stone-gray text-sm">{language === 'ar' ? 'العملاء النشطون' : 'Active Customers'}</p>
                <p className="text-2xl font-bold text-elegant-white">247</p>
                <p className="text-blue-400 text-sm">+8% {language === 'ar' ? 'من الشهر الماضي' : 'from last month'}</p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-stone-gray text-sm">{language === 'ar' ? 'الوحدات المباعة' : 'Units Sold'}</p>
                <p className="text-2xl font-bold text-elegant-white">28</p>
                <p className="text-yellow-400 text-sm">+5% {language === 'ar' ? 'من الشهر الماضي' : 'from last month'}</p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-stone-gray text-sm">{language === 'ar' ? 'المهام المتأخرة' : 'Overdue Tasks'}</p>
                <p className="text-2xl font-bold text-elegant-white">3</p>
                <p className="text-red-400 text-sm">-2 {language === 'ar' ? 'من الأسبوع الماضي' : 'from last week'}</p>
              </div>
        ...
        <div className="mt-6">
                ...
                {salesReports.map((report, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="text-lg font-bold text-elegant-white">{report.period}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'إجمالي المبيعات' : 'Total Sales'}</p>
                        <p className="text-xl font-bold text-desert-gold">{report.totalSales} {language === 'ar' ? 'ريال' : 'SAR'}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'الوحدات المباعة' : 'Units Sold'}</p>
                        <p className="text-xl font-bold text-elegant-white">{report.unitsSold}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'متوسط السعر' : 'Average Price'}</p>
                        <p className="text-lg font-medium text-elegant-white">{report.averagePrice} {language === 'ar' ? 'ريال' : 'SAR'}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'الحجوزات' : 'Bookings'}</p>
                        <p className="text-lg font-medium text-elegant-white">{report.unitsBooked}</p>
                      </div>
                    </div>

                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span className="text-stone-gray text-sm">{language === 'ar' ? 'أفضل مشروع:' : 'Top Project:'}</span>
                        <span className="text-elegant-white text-sm">{report.topProject}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-stone-gray text-sm">{language === 'ar' ? 'أفضل مندوب:' : 'Top Sales Rep:'}</span>
                        <span className="text-elegant-white text-sm">{report.topSalesRep}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {paymentReports.map((report, index) => (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'المحصل' : 'Collected'}</p>
                        <p className="text-xl font-bold text-green-400">{report.totalCollected} {language === 'ar' ? 'ريال' : 'SAR'}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'المعلق' : 'Pending'}</p>
                        <p className="text-xl font-bold text-yellow-400">{report.totalPending} {language === 'ar' ? 'ريال' : 'SAR'}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'المتأخر' : 'Overdue'}</p>
                        <p className="text-lg font-medium text-red-400">{report.totalOverdue} {language === 'ar' ? 'ريال' : 'SAR'}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'معدل التحصيل' : 'Collection Rate'}</p>
                        <p className="text-lg font-medium text-elegant-white">{report.collectionRate}</p>
                      </div>
                    </div>

                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span className="text-stone-gray text-sm">{language === 'ar' ? 'متوسط وقت الدفع:' : 'Avg Payment Time:'}</span>
                        <span className="text-elegant-white text-sm">{report.avgPaymentTime} {language === 'ar' ? 'يوم' : 'days'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-stone-gray text-sm">{language === 'ar' ? 'المدفوعات المتأخرة:' : 'Late Payments:'}</span>
                        <span className="text-red-400 text-sm">{report.latePayments}</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
          ...
          {activeTab === 'satisfaction' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold text-elegant-white">
                  {language === 'ar' ? 'تقارير رضا العملاء' : 'Customer Satisfaction Reports'}
                </h2>
                <motion.button
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                    <div className="text-center">
                      <p className="text-3xl font-bold text-yellow-400">{report.averageRating}</p>
                      <p className="text-stone-gray text-sm">{language === 'ar' ? 'متوسط التقييم' : 'Average Rating'}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-green-400">{report.satisfied}</p>
                      <p className="text-stone-gray text-sm">{language === 'ar' ? 'راضون' : 'Satisfied'}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-yellow-400">{report.neutral}</p>
                      <p className="text-stone-gray text-sm">{language === 'ar' ? 'محايدون' : 'Neutral'}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-red-400">{report.unsatisfied}</p>
                      <p className="text-stone-gray text-sm">{language === 'ar' ? 'غير راضون' : 'Unsatisfied'}</p>
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6">
                {projectPerformance.map((project, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="text-lg font-bold text-elegant-white">{project.project}</h3>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                      <div>
                        ...
                        <p className="text-xl font-bold text-elegant-white">{project.totalUnits}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'مباع' : 'Sold'}</p>
                        <p className="text-xl font-bold text-green-400">{project.soldUnits}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'محجوز' : 'Reserved'}</p>
                        <p className="text-xl font-bold text-yellow-400">{project.reservedUnits}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'متاح' : 'Available'}</p>
                        <p className="text-xl font-bold text-blue-400">{project.availableUnits}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'معدل البيع' : 'Sales Rate'}</p>
                        <p className="text-lg font-medium text-desert-gold">{project.salesRate}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'متوسط السعر' : 'Avg Price'}</p>
                        <p className="text-lg font-medium text-elegant-white">{project.avgPrice}</p>
                      </div>
                      <div>
                        <p className="text-stone-gray text-sm">{language === 'ar' ? 'الإيرادات' : 'Revenue'}</p>
                        <p className="text-lg font-medium text-desert-gold">{project.revenue}</p>
                      </div>
                    </div>
                  </motion.div>
                ))}
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <h3 className="text-lg font-medium text-elegant-white mb-2">{task.description}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                          <div>
                            <span className="text-stone-gray">{language === 'ar' ? 'تاريخ الاستحقاق:' : 'Due Date:'}</span>
                            <span className="text-elegant-white ml-2">{task.dueDate}</span>
                          </div>
                          <div>
                            <span className="text-stone-gray">{language === 'ar' ? 'أيام التأخير:' : 'Days Overdue:'}</span>
                            <span className="text-red-400 ml-2 font-medium">{task.daysOverdue}</span>
                          </div>
                          <div>
                            <span className="text-stone-gray">{language === 'ar' ? 'المكلف:' : 'Assigned To:'}</span>
                            <span className="text-elegant-white ml-2">{task.assignedTo}</span>
                          </div>
                        </div>
                      </div>
                      <motion.button
        ...
        >
          <form className="space-y-6">
            ...
            {selectedDateRange === 'custom' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField label={language === 'ar' ? 'من تاريخ' : 'From Date'} required>
                  <input
                    type="date"
                    className="w-full bg-stone-gray/10 border border-desert-gold/20 rounded-lg px-4 py-3 text-elegant-white focus:outline-none focus:border-desert-gold transition-colors duration-300"
                  />
                ...
                <FormField label={language === 'ar' ? 'إلى تاريخ' : 'To Date'} required>
                  <input
                    type="date"
                    className="w-full bg-stone-gray/10 border border-desert-gold/20 rounded-lg px-4 py-3 text-elegant-white focus:outline-none focus:border-desert-gold transition-colors duration-300"
                  />
