...
const initialRequests: Request[] = [
  {
    ...
    reportedBy: 'مالك',
    history: [
      { date: '2024-06-01', action: 'تم إنشاء الطلب', by: 'العميل' },
    ],
  },
  {
    id: 'REQ-1002',
...
export default function MaintenanceTasksPage() {
  ...
  const statusCounts = useMemo(() => {
    const counts: Record<string, number> = { 'new': 0, 'in-progress': 0, 'completed': 0, 'rejected': 0 };
    requests.forEach(r => {
  ...
  const filteredData = useMemo(() => {
    return requests.filter((req) => {
      ...
      if (projectFilter !== 'all' && req.project !== projectFilter) return false;
      if (assignedFilter !== 'all' && assignedFilter !== 'unassigned' && req.assigned !== assignedFilter) return false;
      if (assignedFilter === 'unassigned' && req.assigned !== 'unassigned') return false;
      if (issueTypeFilter !== 'all' && req.issueType !== issueTypes.find(i => i.value === issueTypeFilter)?.label[language]) return false;
  ...
  }, [requests, activeStatusCard, statusFilter, projectFilter, dateFilter, assignedFilter, issueTypeFilter, language]);
  ...
  const columns = [
    ...
    { key: 'issueType', label: language === 'ar' ? 'نوع المشكلة' : 'Issue Type', render: (value: string) => (
      <StatusBadge status={value} variant={getIssueTypeVariant(value)} size="md" />
    ) },
    { key: 'status', label: language === 'ar' ? 'الحالة' : 'Status', render: (value: string) => {
      const status = statusOptions.find(opt => opt.label[language] === value || opt.value === value);
      return <StatusBadge status={status ? status.label[language] : value} variant={getStatusVariant(status ? status.value : value)} size="md" />;
    } },
    { key: 'date', label: language === 'ar' ? 'التاريخ' : 'Date' },
    { key: 'assigned', label: language === 'ar' ? 'الموظف المسؤول' : 'Assigned Employee' },
    { key: 'actions', label: language === 'ar' ? 'الإجراءات' : 'Actions', render: (_: any, row: any) => (
      <button
        onClick={() => { setSelectedRequest(row); setShowDetail(true); }}
  ...
  function handleFormSubmit(e: any) {
    e.preventDefault();
    if (!form.projectId || !form.unitId || !form.issueType || !form.description || !form.preferredTime || !form.reportedBy) {
    ...
    setRequests(prev => [
      {
        ...
        project: project ? `${project.name} – ${project.type[language]} – ${unit?.number}` : '',
        unit: unit?.number || '',
        issueType: issueTypes.find(i => i.value === form.issueType)?.label[language] || '',
        status: 'new',
        ...
        report: '',
        history: [
          { date: new Date().toISOString().slice(0, 10), action: language === 'ar' ? 'تم إنشاء الطلب' : 'Request Created', by: language === 'ar' ? 'العميل' : 'Client' },
        ],
      },
      ...prev,
    ]);
    setShowForm(false);
    setForm({ projectId: '', unitId: '', issueType: '', description: '', attachment: '', preferredTime: '', reportedBy: '' });
    setFormError('');
  }
  ...
  return (
    <PageWrapper>
          ...
          return (
            <button
              ...
              className={`min-w-[140px] flex flex-col items-center justify-center px-6 py-4 rounded-xl shadow-md font-bold border-2 transition-all duration-200
                ${card.color}
                ${isActive ? 'border-desert-gold shadow-lg' : 'border-transparent hover:border-desert-gold'}
              `}
              style={{ fontFamily: 'inherit' }}
            >
              <span className="text-2xl mb-1">{statusCounts[card.key]}</span>
              <span className="text-sm font-medium">{card.label[language]}</span>
            </button>
          );
        })}
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent className="bg-[#232323]/95 border border-[#C6A15B]/20 rounded-xl shadow-xl text-[#fff]">
                {statusOptions.map(opt => (
                  <SelectItem key={opt.value} value={opt.value} className="hover:bg-[#C6A15B]/10 data-[state=checked]:bg-[#C6A15B]/20 data-[state=checked]:text-[#C6A15B]">
                    {opt.label[language]}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div>
            ...
            <Select value={projectFilter} onValueChange={setProjectFilter}>
              <SelectTrigger className="w-full bg-[#232323]/80 border border-[#C6A15B]/30 rounded-xl shadow-lg text-[#fff] font-bold px-4 py-2 focus:ring-2 focus:ring-[#C6A15B]/40 transition h-12">
                <SelectValue placeholder={projectOptions[0].label[language]} />
              </SelectTrigger>
              <SelectContent className="bg-[#232323]/95 border border-[#C6A15B]/20 rounded-xl shadow-xl text-[#fff]">
          ...
          <div>
            ...
            <Select value={assignedFilter} onValueChange={setAssignedFilter}>
              <SelectTrigger className="w-full bg-[#232323]/80 border border-[#C6A15B]/30 rounded-xl shadow-lg text-[#fff] font-bold px-4 py-2 focus:ring-2 focus:ring-[#C6A15B]/40 transition h-12">
                <SelectValue placeholder={assignedOptions[0].label[language]} />
              </SelectTrigger>
              <SelectContent className="bg-[#232323]/95 border border-[#C6A15B]/20 rounded-xl shadow-xl text-[#fff]">
      ...
      <Modal isOpen={showForm} onClose={() => setShowForm(false)} title={language === 'ar' ? 'طلب صيانة جديد' : 'New Maintenance Request'} size="lg">
        <form onSubmit={handleFormSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField label={language === 'ar' ? 'الوحدة' : 'Unit'} required>
              <SelectContext
                options={mockProjects.map(p => ({ value: p.id, label: { ar: `${p.name} – ${p.type.ar}`, en: `${p.name} – ${p.type.en}` } }))}
              ...
              {form.projectId && (
                <SelectContext
                  options={mockProjects.find(p => p.id === form.projectId)?.units.map(u => ({ value: u.id, label: { ar: `${mockProjects.find(p => p.id === form.projectId)?.name} – ${mockProjects.find(p => p.id === form.projectId)?.type[language]} – ${u.number}`, en: `${mockProjects.find(p => p.id === form.projectId)?.name} – ${mockProjects.find(p => p.id === form.projectId)?.type[language]} – ${u.number}` } })) || []}
                  value={form.unitId}
            ...
            <FormField label={language === 'ar' ? 'الوقت المناسب للزيارة' : 'Preferred Visit Time'} required>
              <Input
                type="time"
            ...
            <FormField label={language === 'ar' ? 'جهة الإبلاغ' : 'Reporting Party'} required>
          ...
          <FormField label={language === 'ar' ? 'وصف المشكلة' : 'Description'} required>
            <textarea
          ...
          <FormField label={language === 'ar' ? 'رفع مرفق (اختياري)' : 'Attachment (Optional)'}>
            <input
              type="file"
              id="attachment-upload"
              accept="image/*,video/*"
              onChange={e => handleFormChange('attachment', e.target.files?.[0] || '')}
              className="hidden"
            />
            <UploadLabel
              htmlFor="attachment-upload"
              label={language === 'ar' ? 'رفع مرفق (اختياري)' : 'Attachment (Optional)'}
              withBorder
            />
          </FormField>
      ...
      <Modal isOpen={showDetail} onClose={() => setShowDetail(false)} title={language === 'ar' ? 'تفاصيل طلب الصيانة' : 'Maintenance Request Details'} size="xl">
        {selectedRequest && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="text-elegant-white font-bold mb-4">{selectedRequest.unit}</div>
                <div className="mb-2 text-stone-gray text-sm font-medium">
                  {language === 'ar' ? 'المشروع' : 'Project'}
                </div>
                <div className="text-elegant-white mb-4">{selectedRequest.project}</div>
                <div className="mb-2 text-stone-gray text-sm font-medium">
                  {language === 'ar' ? 'نوع المشكلة' : 'Issue Type'}
                </div>
                <div className="text-elegant-white mb-4">{selectedRequest.issueType}</div>
                <div className="mb-2 text-stone-gray text-sm font-medium">
                  {language === 'ar' ? 'الوصف' : 'Description'}
                </div>
                <div className="text-elegant-white mb-4">{selectedRequest.description}</div>
                <div className="mb-2 text-stone-gray text-sm font-medium">
                  {language === 'ar' ? 'الوقت المناسب للزيارة' : 'Preferred Visit Time'}
                </div>
                <div className="text-elegant-white mb-4">{selectedRequest.preferredTime || (language === 'ar' ? 'غير محدد' : 'Not specified')}</div>
                <div className="mb-2 text-stone-gray text-sm font-medium">
                  {language === 'ar' ? 'جهة الإبلاغ' : 'Reported By'}
                </div>
                <div className="text-elegant-white mb-4">{selectedRequest.reportedBy}</div>
                <div className="mb-2 text-stone-gray text-sm font-medium">
                  {language === 'ar' ? 'المرفق' : 'Attachment'}
                </div>
                <div className="text-elegant-white mb-4">
                  {selectedRequest.attachment ? selectedRequest.attachment : (language === 'ar' ? 'لا يوجد' : 'None')}
                </div>
              </div>
              {/* Admin Panel */}
              <div>
                <FormField label={language === 'ar' ? 'الحالة' : 'Status'}>
                  <SelectContext
                    ...
                    onChange={value => setSelectedRequest({ ...selectedRequest, status: value })}
                    placeholder={language === 'ar' ? 'اختر الحالة' : 'Select Status'}
                    language={language}
                  />
                </FormField>
                <FormField label={language === 'ar' ? 'الموظف المسؤول' : 'Assigned Employee'}>
                  <SelectContext
                    options={assignedOptions}
                    value={selectedRequest.assigned}
                    onChange={value => setSelectedRequest({ ...selectedRequest, assigned: value })}
                ...
                <FormField label={language === 'ar' ? 'تقرير الفني' : 'Technician Report'}>
                  <input
                    type="file"
                    accept="image/*,video/*"
                    onChange={e => setSelectedRequest(selectedRequest ? { ...selectedRequest, report: e.target.files?.[0]?.name || '' } : selectedRequest)}
                    className="w-full bg-stone-gray/10 border border-desert-gold/20 rounded-lg px-4 py-2 text-elegant-white mb-4 focus:outline-none focus:border-desert-gold transition-colors duration-300"
                  />
                </FormField>
                <div className="flex gap-4 mt-4">
                  <button
                    onClick={() => {
                      setRequests(reqs => reqs.map(r => r.id === selectedRequest.id ? selectedRequest : r));
                      setShowDetail(false);
                    }}
            ...
            {/* History Log */}
            <div>
              <h3 className="text-lg font-bold text-elegant-white mb-2">{language === 'ar' ? 'سجل الطلب' : 'Request History'}</h3>
              <div className="bg-stone-gray/10 rounded-lg p-4 border border-desert-gold/20">
                {selectedRequest.history && selectedRequest.history.length > 0 ? (
                  <ul className="space-y-2">
                    {selectedRequest.history.map((h, i) => (
                      <li key={i} className="flex items-center justify-between text-sm text-stone-gray">
                        <span>{h.date}</span>
                        <span>{h.action}</span>
                        <span>{h.by}</span>
                      </li>
                  ...
                  <div className="text-stone-gray text-sm">{language === 'ar' ? 'لا يوجد سجل' : 'No history'}</div>
                )}
              </div>
            </div>
          </div>
