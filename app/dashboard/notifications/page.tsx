...
import { Plus, Send, MessageSquare, Bell, Users, Calendar, CheckCircle, Clock, User, Search, Filter, Eye, Edit, Trash2 } from 'lucide-react';
...
export default function NotificationsPage() {
  ...
  const communications = [
    {
      ...
      subject: language === 'ar' ? 'استفسار عن التشطيبات' : 'Inquiry about finishes',
      date: '2024-01-22',
      time: '10:30',
      duration: '15 دقيقة',
      notes: language === 'ar' ? 'العميل راضي عن التقدم ويريد تحديث في التشطيبات' : 'Client satisfied with progress, wants update on finishes',
      followUp: '2024-01-25',
    ...
    },
    {
      ...
      time: '14:15',
      duration: null,
      notes: language === 'ar' ? 'طلب معاينة الوحدة V15 يوم الخميس' : 'Requested viewing of unit V15 on Thursday',
      followUp: '2024-01-24',
    ...
    },
    {
      ...
      subject: language === 'ar' ? 'مناقشة خطة الدفع' : 'Payment plan discussion',
      date: '2024-01-21',
      time: '16:00',
      duration: '45 دقيقة',
      notes: language === 'ar' ? 'تم الاتفاق على خطة دفع جديدة بأقساط شهرية' : 'Agreed on new payment plan with monthly installments',
      followUp: null,
      staff: language === 'ar' ? 'فاطمة الحربي' : 'Fatima Al-Harbi'
    }
  ];
  ...
  const reminders = [
    {
      id: 1,
      title: language === 'ar' ? 'متابعة دفعة أحمد العتيبي' : 'Follow up Ahmed Al-Otaibi payment',
      description: language === 'ar' ? 'تذكير بالدفعة المستحقة للوحدة A101' : 'Reminder for due payment for unit A101',
      dueDate: '2024-01-25',
    ...
    {
      id: 2,
      title: language === 'ar' ? 'اجتماع مع خالد المطيري' : 'Meeting with Khalid Al-Mutairi',
      description: language === 'ar' ? 'مناقشة تفاصيل العقد النهائي' : 'Discuss final contract details',
      dueDate: '2024-01-24',
    ...
    {
      id: 3,
      title: language === 'ar' ? 'تسليم مفاتيح الوحدة C302' : 'Unit C302 key handover',
      description: language === 'ar' ? 'تسليم المفاتيح لنورا السالم' : 'Key handover to Nora Al-Salem',
      dueDate: '2024-01-26',
  ...
  const internalMessages = [
    {
      ...
      message: language === 'ar' ? 'يرجى تحديث حالة العميل خالد المطيري إلى "عقد موقع"' : 'Please update Khalid Al-Mutairi status to "Contract Signed"',
    ...
    },
    {
      ...
      subject: language === 'ar' ? 'تحديث نظام الصيانة' : 'Maintenance system update',
      message: language === 'ar' ? 'تم تحديث نظام إدارة طلبات الصيانة، يرجى مراجعة الدليل الجديد' : 'Maintenance management system updated, please review new guide',
  ...
  const notifications = [
    {
      ...
      message: language === 'ar' ? 'تم إنشاء حجز جديد للعميل سعد الدوسري' : 'New booking created for client Saad Al-Dosari',
    ...
    },
    {
      ...
      message: language === 'ar' ? 'تم استلام دفعة من أحمد العتيبي بقيمة 250,000 ريال' : 'Payment received from Ahmed Al-Otaibi for 250,000 SAR',
    ...
    },
    {
      ...
      message: language === 'ar' ? 'طلب صيانة جديد من فاطمة الحربي للوحدة B205' : 'New maintenance request from Fatima Al-Harbi for unit B205',
  ...
  const tabs = [
    { id: 'communications', label: language === 'ar' ? 'سجل التواصل' : 'Communication Log' },
    { id: 'reminders', label: language === 'ar' ? 'التذكيرات' : 'Task Reminders' },
    { id: 'messages', label: language === 'ar' ? 'الرسائل الداخلية' : 'Internal Messages' },
    { id: 'notifications', label: language === 'ar' ? 'مركز الإشعارات' : 'Notification Center' }
  ];
    ...
    <PageWrapper>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-3xl font-bold text-elegant-white">
              {language === 'ar' ? 'الإشعارات والتواصل' : 'Notifications & Communication'}
            </h1>
            <p className="text-stone-gray mt-1">
              {language === 'ar' ? 'إدارة التواصل والإشعارات والتذكيرات' : 'Manage communications, notifications, and reminders'}
            </p>
          </div>
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold text-elegant-white">
                  {language === 'ar' ? 'سجل التواصل مع العملاء' : 'Client Communication Log'}
                </h2>
                <motion.button
                ...
                >
                  ...
                  <span>{language === 'ar' ? 'إضافة تواصل' : 'Add Communication'}</span>
                </motion.button>
              </div>
              ...
              <DataTable
                ...
                data={communications}
                searchPlaceholder={language === 'ar' ? 'البحث في سجل التواصل...' : 'Search communication log...'}
                onView={handleView}
                onEdit={handleEdit}
          ...
          {activeTab === 'reminders' && (
            <div className="space-y-6">
              ...
              <DataTable
                ...
                data={reminders}
                searchPlaceholder={language === 'ar' ? 'البحث في التذكيرات...' : 'Search reminders...'}
                onView={handleView}
                onEdit={handleEdit}
                onDelete={handleDelete}
              />
          ...
          {activeTab === 'messages' && (
            <div className="space-y-6">
              ...
              <DataTable
                ...
                searchPlaceholder={language === 'ar' ? 'البحث في الرسائل...' : 'Search messages...'}
                onView={handleView}
                onEdit={handleEdit}
                onDelete={handleDelete}
              />
                ...
                {notifications.map((notification, index) => (
                  <motion.div
                    ...
                    className={`bg-obsidian/50 backdrop-blur-sm rounded-lg p-6 border transition-all duration-300 hover:border-desert-gold/40 ${
                      notification.read ? 'border-desert-gold/20' : 'border-desert-gold/40 bg-desert-gold/5'
                    }`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-4 rtl:space-x-reverse flex-1">
                        <div className={`p-2 rounded-lg ${
                          ...
                          {notification.type === 'booking' ? <Calendar className="h-5 w-5" /> :
                           notification.type === 'payment' ? <CheckCircle className="h-5 w-5" /> :
                           <Bell className="h-5 w-5" />}
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 rtl:space-x-reverse mb-1">
                            <h3 className="text-lg font-medium text-elegant-white">{notification.title}</h3>
                            {!notification.read && (
                          ...
                          <p className="text-stone-gray mb-2">{notification.message}</p>
        ...
        <Modal
          ...
          title={language === 'ar' ? 'رسالة داخلية جديدة' : 'New Internal Message'}
          size="lg"
        >
          <form className="space-y-6">
            ...
            <FormField label={language === 'ar' ? 'الموضوع' : 'Subject'} required>
              <input
                type="text"
            ...
            <FormField label={language === 'ar' ? 'الرسالة' : 'Message'} required>
              <textarea
                rows={6}
        ...
        >
          <form className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <FormField label={language === 'ar' ? 'العنوان' : 'Title'} required>
                <input
                  type="text"
            ...
            <FormField label={language === 'ar' ? 'الوصف' : 'Description'} required>
              <textarea
                rows={4}
                defaultValue={selectedItem?.description || ''}
                className="w-full bg-stone-gray/10 border border-desert-gold/20 rounded-lg px-4 py-3 text-elegant-white placeholder-stone-gray focus:outline-none focus:border-desert-gold transition-colors duration-300 resize-none"
                placeholder={language === 'ar' ? 'أدخل وصف التذكير...' : 'Enter reminder description...'}
              />
            </FormField>
        ...
        <Modal
          ...
          title={selectedItem ? 
            (language === 'ar' ? 'تعديل التواصل' : 'Edit Communication') : 
            (language === 'ar' ? 'إضافة تواصل جديد' : 'Add New Communication')
          }
          size="lg"
        >
          <form className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <FormField label={language === 'ar' ? 'نوع التواصل' : 'Communication Type'} required>
                <select
              ...
              <FormField label={language === 'ar' ? 'التاريخ' : 'Date'} required>
                <input
                  type="date"
            ...
            <FormField label={language === 'ar' ? 'الموضوع' : 'Subject'} required>
              <input
                ...
                className="w-full bg-stone-gray/10 border border-desert-gold/20 rounded-lg px-4 py-3 text-elegant-white placeholder-stone-gray focus:outline-none focus:border-desert-gold transition-colors duration-300"
                placeholder={language === 'ar' ? 'أدخل موضوع التواصل' : 'Enter communication subject'}
              />
            </FormField>

            <FormField label={language === 'ar' ? 'الملاحظات' : 'Notes'} required>
              <textarea
                rows={4}
                defaultValue={selectedItem?.notes || ''}
                className="w-full bg-stone-gray/10 border border-desert-gold/20 rounded-lg px-4 py-3 text-elegant-white placeholder-stone-gray focus:outline-none focus:border-desert-gold transition-colors duration-300 resize-none"
                placeholder={language === 'ar' ? 'أدخل ملاحظات التواصل...' : 'Enter communication notes...'}
              />
            </FormField>

            <FormField label={language === 'ar' ? 'تاريخ المتابعة' : 'Follow Up Date'}>
              <input
                type="date"
            ...
            <div className="flex justify-end space-x-4 rtl:space-x-reverse pt-6">
              ...
              >
                {selectedItem ? 
                  (language === 'ar' ? 'تحديث' : 'Update') : 
                  (language === 'ar' ? 'إضافة التواصل' : 'Add Communication')
                }
              </motion.button>
            </div>
          </form>
