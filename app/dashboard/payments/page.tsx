...
import { Plus, Upload, Download, Send, DollarSign, Calendar, FileText, TrendingUp, Filter, Eye, Edit, AlertCircle } from 'lucide-react';
...
export default function PaymentsPage() {
  ...
  const tabs = [
    { id: 'schedule', label: language === 'ar' ? 'جدول المدفوعات' : 'Payment Schedule' },
    { id: 'transfers', label: language === 'ar' ? 'تأكيد التحويلات' : 'Bank Transfers' },
    { id: 'upcoming', label: language === 'ar' ? 'المدفوعات القادمة' : 'Upcoming Payments' },
    { id: 'invoices', label: language === 'ar' ? 'الفواتير والإيصالات' : 'Invoices & Receipts' },
    { id: 'reports', label: language === 'ar' ? 'التقارير المالية' : 'Financial Reports' }
  ];
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-stone-gray mt-1">
              {language === 'ar' ? 'إدارة المدفوعات والفواتير والتحويلات البنكية' : 'Manage payments, invoices, and bank transfers'}
            </p>
          </div>
          <div className="flex space-x-3 rtl:space-x-reverse">
            ...
            >
              ...
              <span>{language === 'ar' ? 'تأكيد تحويل' : 'Confirm Transfer'}</span>
            </motion.button>
          </div>
        </div>
        ...
        <div className="mt-6">
          {activeTab === 'schedule' && (
            <DataTable
              ...
              searchPlaceholder={language === 'ar' ? 'البحث في المدفوعات...' : 'Search payments...'}
              onView={handleView}
              onEdit={handleEdit}
                ...
                {upcomingPayments.map((payment, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-lg font-bold text-elegant-white">{payment.client}</h3>
                        <p className="text-stone-gray">{language === 'ar' ? 'الوحدة:' : 'Unit:'} {payment.unit}</p>
                      </div>
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                        payment.daysLeft <= 3 ? 'bg-red-500 text-white' : 
                        payment.daysLeft <= 7 ? 'bg-yellow-500 text-deep-black' : 
                    ...
                    <div className="space-y-3 mb-4">
                      ...
                      <div className="flex items-center space-x-2 rtl:space-x-reverse">
                        ...
                        <span className="text-stone-gray">
                          {language === 'ar' ? 'تاريخ الاستحقاق:' : 'Due Date:'} {payment.dueDate}
                        </span>
                      </div>
                    </div>
          ...
          {activeTab === 'invoices' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold text-elegant-white">
                  {language === 'ar' ? 'الفواتير والإيصالات' : 'Invoices & Receipts'}
                </h2>
                <motion.button
                  onClick={() => setShowInvoiceModal(true)}
                  className="bg-desert-gold text-deep-black px-4 py-2 rounded-lg font-medium hover:bg-warm-sand transition-all duration-300 flex items-center space-x-2 rtl:space-x-reverse"
              ...
              <DataTable
                ...
                data={invoices}
                searchPlaceholder={language === 'ar' ? 'البحث في الفواتير...' : 'Search invoices...'}
                onView={handleView}
                onEdit={handleEdit}
                onDelete={handleDelete}
              />
            ...
            <div className="bg-obsidian/50 backdrop-blur-sm rounded-xl p-8 border border-desert-gold/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                <div className="bg-stone-gray/10 rounded-lg p-6 text-center">
                  ...
                  <p className="text-2xl font-bold text-desert-gold">2.5M {language === 'ar' ? 'ريال' : 'SAR'}</p>
                </div>
                ...
                <div className="bg-stone-gray/10 rounded-lg p-6 text-center">
                  ...
                  <p className="text-2xl font-bold text-yellow-500">850K {language === 'ar' ? 'ريال' : 'SAR'}</p>
                </div>
                
                <div className="bg-stone-gray/10 rounded-lg p-6 text-center">
                  <Calendar className="h-8 w-8 text-red-500 mx-auto mb-3" />
                  <h3 className="text-lg font-bold text-elegant-white mb-2">
                    {language === 'ar' ? 'المدفوعات المتأخرة' : 'Overdue Payments'}
                  </h3>
                  <p className="text-2xl font-bold text-red-500">150K {language === 'ar' ? 'ريال' : 'SAR'}</p>
                </div>
              </div>
        ...
        >
          <div className="text-center py-8">
            ...
            <p className="text-stone-gray">
              {language === 'ar' ? 'يرجى ملء النموذج أعلاه لتأكيد التحويل البنكي' : 'Please fill the form above to confirm bank transfer'}
            </p>
          </div>
        </Modal>
